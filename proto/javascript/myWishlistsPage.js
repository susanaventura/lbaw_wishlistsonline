...
function setupDropzone(){
	...
	$('.img-upload > .thumbnail').each(function () {
		...
		if($(this).children().size() == 0){
			...
			var tempZone = dropzoneItem.dropzone({ url: "../actions/uploadImage.php?type=wishlistItem", thumbnailWidth: 210, thumbnailHeight: 220, maxFiles: 1, maxfilesexceeded: function(file) {this.removeAllFiles(); this.addFile(file);}});
		
      // If user has image //
      if($('#wishlistItemHasImage'+i).exists()){
        
        var img_location = '../images/wishlistItems/' + $('#wishlistItemHasImage'+i).attr('img');
        var thumb_location = '../images/wishlistItems/thumb_' + $('#wishlistItemHasImage'+i).attr('img');
        var file_to_add = { name: $('#wishlistItemHasImage'+i).attr('img'), size: 23456 };
        var myDropzone = tempZone[0].dropzone;
        
        $('#wishlistItemHasImage'+i).remove();
